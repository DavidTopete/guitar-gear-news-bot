...
# Idioma de origen POR DEFECTO. Cada llamada puede sobreescribirlo con el
# parametro `origen` (p. ej. "ja" para fuentes japonesas, "auto" para
...
def _clave_cache(texto, origen):
    base = f"{origen}|{IDIOMA_DESTINO}|{texto}"
...
def _p_deepl(texto, origen):
    ...
    r = requests.post(
        ...
        data={
            ...
            **({"source_lang": origen.upper()} if origen != "auto" else {}),
        },
        timeout=TIMEOUT,
    )
...
def _p_libretranslate(texto, origen):
...
def _p_mymemory(texto, origen):
    parametros = {"q": texto, "langpair": f"{origen}|{IDIOMA_DESTINO}"}
...
def _p_lingva(texto, origen):
    base = os.getenv("LINGVA_URL", "https://lingva.ml").rstrip("/")
    url = f"{base}/api/v1/{origen}/{IDIOMA_DESTINO}/{urllib.parse.quote(texto)}"
...
def _p_google_clients5(texto, origen):
    r = requests.get(
...
def _p_google_gtx(texto, origen):
    r = requests.get(
...
def _p_deep_translator(texto, origen):
    try:
        from deep_translator import GoogleTranslator
    except Exception as error:
        raise TraduccionFallida(f"deep_translator no disponible: {error}")

    resultado = GoogleTranslator(source=origen, target=IDIOMA_DESTINO).translate(texto)
...
def _traducir_con_proveedor(nombre, texto_completo, origen, validador=None):
    ...
    for indice, chunk in enumerate(chunks, start=1):
        ...
        for intento in range(1, MAX_INTENTOS + 1):
            try:
                parcial = funcion(chunk, origen)

                # Validacion semantica opcional (p. ej. rechazar japones
                # residual). Un fallo aqui NO es error de red: significa que
                # este proveedor devolvio algo inservible, asi que se trata
                # como fallo del proveedor y la cascada prueba el siguiente.
                if validador is not None and not validador(parcial):
                    raise TraduccionFallida(
                        f"{nombre}: la salida no paso la validacion semantica"
                    )
...
def _traducir_interno(texto, origen=None, usar_cache=True, validador=None):
    ...
    for nombre in _orden_proveedores():
        ...
        try:
            resultado = _traducir_con_proveedor(nombre, texto, origen, validador)

            STATS["traducciones"] += 1
            STATS["proveedor_usado"][nombre] = STATS["proveedor_usado"].get(nombre, 0) + 1
            log.info(f"Traducido con '{nombre}' ({origen}->{IDIOMA_DESTINO}, {len(texto)} chars)")
    ...
    STATS["fallos"] += 1
    log.error(f"Todos los proveedores fallaron ({origen}, {len(texto)} chars).")
    return False, texto


def traducir(texto, origen=None, usar_cache=True, validador=None):
    """Modo tolerante: si todos los proveedores fallan, devuelve el texto
    original (nunca basura ni HTML de error)."""
    _, resultado = _traducir_interno(texto, origen, usar_cache, validador)
    return resultado


def traducir_estricto(texto, origen=None, usar_cache=True, validador=None):
    """Modo estricto: lanza TraduccionFallida si no se pudo traducir.

    `validador` permite rechazar salidas sintacticamente correctas pero
    semanticamente invalidas (por ejemplo, texto japones sin traducir).
    """
    exito, resultado = _traducir_interno(texto, origen, usar_cache, validador)

    if not exito:
        raise TraduccionFallida(
            f"texto de {len(texto)} chars ({origen or IDIOMA_ORIGEN_DEFAULT}) "
            f"no traducible por ningun proveedor"
        )
...
def autotest():
    ...
    for nombre in _orden_proveedores():
        if nombre in _PROVEEDORES_QUEMADOS:
            continue
        try:
            resultado = _traducir_con_proveedor(nombre, muestra, IDIOMA_ORIGEN_DEFAULT)
